...
class Solution(object):
    ...
    def getNumPos_v3(self, b, num_i, num_cnt, order):
        num = None
        for i in range(9):
            n = order[i]
            if num_cnt[n-1] < 9:
        ...
        best_i_j_list = min(pos);
        return (num,best_i_j_list)


    # python when to add self
    def search(self, cnt0, b, num_i, num_cnt, order):
        if cnt0 == 0:
            return True

        # num,i_j_list = self.getNumPos(b)
        # num,i_j_list = self.getNumPos_v2(b)
        num,i_j_list = self.getNumPos_v3(b, num_i, num_cnt, order)
        ...
        for j in i_j_list[2]:
            b[i][j] = num
            num_i[num-1][i] = j; num_cnt[num-1] += 1

            self.print_b(b, i, j)
            # input("Press Enter to continue...")
            if self.search(cnt0-1, b, num_i, num_cnt, order):
                return True

            b[i][j] = 0
            num_i[num-1][i] = -1; num_cnt[num-1] -= 1
    ...
    def solveSudoku(self, board):
        b = [[0 for j in range(9)] for i in range(9)]
        ...
        num_i = [[-1 for i in range(9)] for n in range(9)]
        num_cnt = [0 for n in range(9)]
        for i in range(9):
            for j in range(9):
                if b[i][j] != 0:
                    n = b[i][j];
        ...
        order = [x for _,x in sorted(zip(num_cnt, range(1,10)), reverse=True)];
        print(sorted(zip(num_cnt, range(1,10)), reverse=True), order)

        if self.search(cnt0, b, num_i, num_cnt, order):
            pass
        else:
            print('Fail')
